import { Entity, Species } from "./entity.js";
...
const applyEdgeVelocityBoost = (entity: Entity, width: number, height: number) => {
  ...
  for (let i = 0; i < entities.length; i++) {
    for (let j = i + 1; j < entities.length; j++) {
      ...
      if (dist < a.radius + b.radius) {
        ...
        if (winner !== null) {
          if (winner === a.species) b.species = a.species;
          else a.species = b.species;
        }
      }
    }
  }
