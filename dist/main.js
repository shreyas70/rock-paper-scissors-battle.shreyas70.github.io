...
let gameState = "entity-selection";
let userSelectedEntity = null;
...
const entityChoices = document.querySelectorAll(".entity-choice");
const updateUI = () => {
...
const updateBlastCooldown = () => {
  ...
  const remaining = Math.max(
    0,
    BUTTON_BLAST_COOLDOWN - (currentTime - lastButtonBlastTime),
  );
  const seconds = Math.ceil(remaining / 1000);
  if (remaining > 0) {
...
const handleEntitySelection = (selectedEntity) => {
  ...
  gameInstruction.textContent =
    "Strategically tap to create ripples and secure your win!";
  // Start the game
  init();
  startGame();
};
...
document.getElementById("restart").addEventListener("click", () => {
  ...
  gameInstruction.textContent =
    "Pick Rock/Paper/Scissors \n Then strategically tap to create ripples and secure your win!";
...
const handleForceBlast = (e) => {
  ...
  console.log(
    `Converted to canvas: ${canvasX.toFixed(1)}, ${canvasY.toFixed(1)}`,
  );
  // Apply force blast
  ...
  console.log(
    `Canvas force blast applied at: ${canvasX.toFixed(1)}, ${canvasY.toFixed(1)}`,
  );
};
...
blastButton.addEventListener("click", () => {
  ...
  console.log(
    `Button force blast applied at: ${randomX.toFixed(1)}, ${randomY.toFixed(1)}`,
  );
});
